...
class Obstacle(): # Clase de detección de obstáculos 
    def __init__(self): # Incialización de la clase.
        self.min_dist_publisher=rospy.Publisher('/tb3_1/dist_min', Float64, queue_size=10) # Variable publicadora de distancia mínima como tópico.
    ...
    def get_scan(self): # Método para escanear las distancias que da el lidar.
        msg = rospy.wait_for_message("/tb3_1/scan", LaserScan) # Espera a la llegada de una nueva muestra.
...
def main():
    rospy.init_node('turtlebot3_1_distance') #Inicializa el nodo.
    try:
